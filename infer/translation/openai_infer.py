...
{definition}
Your task:
If the original text is in Chinese, translate this pun into English while preserving the original pun effect or creating a new pun in the target language.
If the original text is in English, translate this pun into Chinese while preserving the original pun effect or creating a new pun in the target language.
...
def gpt(input_sentence, index):
    try:
        ...
        return {"index": index, "pred_tag": answer}
...
output_jsonl_path = f'output/{model}/translation/{lan}_{type}_translation_vanilla.jsonl'
