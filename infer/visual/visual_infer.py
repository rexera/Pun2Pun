...
TAG = f'''
{definition}
请判断图中的这一句是同音双关还是同形双关。同音双关输出phonic，同形双关输出graphic。仅需要输出一个单词。后面不需要解释。
'''

EXPLANATION = f'''
{definition}
请解释这句双关语的机制，同音双关需说明如何读音相近或相同，同形双关需说明如何构成一词多义。
'''

APPRECIATION = f'''
{definition}
Please explain the image-text relationship, cultural background, and usage scenarios of this pun.
'''

TRANSLATION = f'''
{definition}
你的任务：根据图文语境，
将这句双关语翻译成【中文】，并尽可能保持原文的双关效果或在目标语中再造一个双关。
...
def process_single_image(image_path):
    ...
    try:
        ...
        # 依次执行四个任务
        tag = gpt(image_path, TAG)
        explanation = gpt(image_path, EXPLANATION)
        appreciation = gpt(image_path, APPRECIATION)
        translation = gpt(image_path, TRANSLATION)
...
output_jsonl_path = f"visual_output/{model}/{lan}_vanilla.jsonl"
