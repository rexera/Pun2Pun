...
def evaluate_puns(pred_file: str, true_file: str, output_file: str, batch_size: int = 10, max_workers: int = 5) -> Dict:
    ...
    anno_dict = {item.get('index', i): item.get('pun', '') for i, item in enumerate(anno_data)}
...
def main():
    ...
    # File paths
    pred_file = f"output/{model}/{lan}_{type}_pun_pred_{strategy}.jsonl"
    true_file = f"data/textual/{lan}/{lan}_{type}_pun.jsonl"
    output_file = f"eval/output/{model}/{lan}_{type}_pun_{strategy}.jsonl"
